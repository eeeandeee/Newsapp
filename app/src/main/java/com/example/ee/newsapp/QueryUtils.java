...
class QueryUtils {
    ...
    public static List<NewArticle> fetchData(String queryUrl) {
    ...
    private static List<NewArticle> parseJSON(String jsonString) {
        if (TextUtils.isEmpty(jsonString)) return null;
        List<NewArticle> out = new ArrayList<>();
        try {
            ...
            for (int i = 0; i < results.length(); ++i) {
                ...
                NewArticle tmpArticle = new NewArticle(title, webUrl, sectionName, date);
                if (fields != null) {
